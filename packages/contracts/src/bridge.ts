...
export const BridgeRequestSchema = z.discriminatedUnion('type', [
  z.object({
    ...
    id: IdSchema,
    type: z.literal('bridge.handshake'),
    payload: z.object({ webVersion: z.string().min(1).max(64) })
  }),
  z.object({
    version: z.literal(NEKO_BRIDGE_VERSION),
    id: IdSchema,
    type: z.literal('navigation.routeChanged'),
    payload: z.object({ route: RouteSchema })
  }),
  z.object({
    version: z.literal(NEKO_BRIDGE_VERSION),
    id: IdSchema,
    type: z.literal('player.open'),
    payload: z.object({ episodeId: IdSchema })
  }),
  z.object({
    version: z.literal(NEKO_BRIDGE_VERSION),
    id: IdSchema,
    ...
    payload: z.object({
      ...
      placement: z.string().min(1).max(128).optional()
    })
  })
]);
...
export const BridgeNativeEventSchema = z.discriminatedUnion('type', [
  z.object({
    ...
    payload: z.object({
      ...
      bridgeVersion: z.literal(NEKO_BRIDGE_VERSION),
      capabilities: z.array(z.string())
    })
  }),
  z.object({
    version: z.literal(NEKO_BRIDGE_VERSION),
    type: z.literal('navigation.navigate'),
    payload: z.object({ route: RouteSchema })
  }),
  z.object({
    version: z.literal(NEKO_BRIDGE_VERSION),
    type: z.literal('player.closed'),
    payload: z.object({ episodeId: IdSchema.optional() }).optional()
  })
]);
...
// Compatibility alias while consumers migrate to the v1 request name.
