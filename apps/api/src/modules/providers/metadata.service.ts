...
export class MetadataService {
  ...
  private async upsert(input:CanonicalAnimeInput){
    ...
    return this.database.db.transaction(async(tx)=>{ const [created]=await tx.insert(anime).values({slug,title:input.title,titleEnglish:input.titleEnglish,titleRomaji:input.titleRomaji,titleNative:input.titleNative,synopsis:input.synopsis,type:input.type ?? 'tv',status:input.status ?? 'unknown',year:input.year,scoreBasisPoints:input.score?Math.round(input.score*100):null,genres:input.genres ?? []}).returning(); if (!created) throw new Error('Anime não foi criado'); await tx.insert(animeExternalIds).values({animeId:created.id,provider:input.provider,externalId:input.externalId}); return created; });
  }
}
