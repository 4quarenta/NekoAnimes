  ...
  return (
    <AppScreen>
        ...
        {watching.data?.length ? (
          <div className="neko-list">
            {watching.data.map((item) => {
              ...
              return <TextRow key={item.episodeId} title={item.title} meta={`T${item.seasonNumber} · Episódio ${item.episodeNumber}`} trailing={`${pct}%`} onClick={() => void navigate({ to: '/anime/$slug', params: { slug: item.slug } })} />;
            })}
          </div>
      ...
      <Section title="Minha lista">
        ...
        {library.data?.length ? (
          <div className="neko-list">
            {library.data.map((item) => <TextRow key={item.animeId} title={item.title} meta={[item.year, item.genres[0], labelStatus(item.status)].filter(Boolean).join(' · ')} trailing="›" onClick={() => void navigate({ to: '/anime/$slug', params: { slug: item.slug } })} />)}
          </div>
