...
import { fetchAnime, fetchEpisodes } from '../lib/api';
        ...
        <div className="neko-season-tabs">
          {item.seasons.map((season) => (
            <button
              type="button"
              key={season.id}
              className={season.id === selectedSeasonId ? 'is-active' : ''}
              onClick={() => { setSeasonId(season.id); setVisible(10); }}
            >
          ...
          <>
            <div className="neko-list">
              {episodeQuery.data.items.map((episode) => (
                <TextRow
                  key={episode.id}
                  title={`${episode.number}. ${episode.title ?? 'Episódio'}`}
                  meta={episode.durationSeconds ? `${Math.round(episode.durationSeconds / 60)} min` : undefined}
                  trailing="▶"
                  onClick={() => NekoNative.player.open(episode.id)}
                />
              ))}
            </div>
            {episodeQuery.data.items.length < episodeQuery.data.total ? (
              <button className="neko-more" type="button" onClick={() => setVisible((value) => value + 10)}>Mostrar mais</button>
            ) : null}
          </>
        ) : null}
      </Section>
