...
class AppManifestRepository(context: Context) {
            ...
            .recoverCatching { error ->
                val cached = preferences.getString(CACHE_KEY, null)
                    ?: throw IllegalStateException(
                        "Configuração remota indisponível e nenhum cache válido foi encontrado",
                        error
                    )
                parse(cached)
            }
    ...
    private fun parse(raw: String): AppManifest {
        ...
        return AppManifest(
            ...
            configVersion = json.getInt("configVersion"),
            mode = mode,
            webAppUrl = webAppUrl,
    ...
    private fun isTrustedWebAppUrl(url: String): Boolean {
        ...
        return candidate.scheme == allowed.scheme &&
            candidate.host == allowed.host &&
            normalizedPort(candidate) == normalizedPort(allowed)
    }
